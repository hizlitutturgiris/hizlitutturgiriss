(() => {
  ...
  function resize() {
    ...
    // ilk açılışta temizle (bazı cihazlarda siyah blok gibi görünmesin)
  ...
  // ✅ Daha sık şimşek: 350-900ms arası (bazen çoklu)
  let nextStrikeAt = performance.now() + rand(350, 900);

  function tick(now) {
    // trail fade (tam siyaha basmadan, izleri yumuşak sil)
    ...
    if (now >= nextStrikeAt) {
      ...
      doFlash(rand(0.18, 0.36));
      nextStrikeAt = now + rand(350, 1000);
    }
